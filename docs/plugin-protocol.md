# RepoAudit plugin protocol (v1)

This document is the contract for writing an external RepoAudit plugin. It's aimed at someone who has never read RepoAudit's Go source — everything a plugin author needs is here.

## Why a separate process, not a Go plugin

RepoAudit plugins run as a **separate process**, communicating over stdin/stdout. Not a native Go plugin (`plugin` package), and this isn't a stopgap — it's a permanent decision, for two independent reasons:
...
- **Stability**: a panic or infinite loop in a same-process plugin takes down all of RepoAudit — there's no way to forcibly stop a stuck goroutine without killing the whole process. A subprocess that crashes or hangs is just an error the host can detect and recover from.
...
1. RepoAudit starts your plugin executable once per scan (not once per file — spawning a process per file would be far too slow on a large repo).
2. RepoAudit sends a `hello` message on your stdin. You reply with `hello_ack` on your stdout.
3. For each eligible file in the repo (already filtered by RepoAudit's own size/binary/vendored-path rules — you never see files larger than 2 MiB, binary files, or anything under `vendor/`/`node_modules/`), RepoAudit sends one `file` message and waits for exactly one `result` message back before sending the next file. One line of JSON per message (NDJSON), synchronous, no concurrent requests.
4. When there are no more files, RepoAudit closes your stdin. Exit when you see EOF.

## Byte-only contract — read this before writing any file I/O in your plugin

**A `file` message gives you the file's content directly (base64-encoded). It never gives you a path you're expected to open yourself.** The `path` field is metadata for your own findings' context (e.g. to decide "this looks like a test file") — it is not a filesystem location you can resolve, because RepoAudit gives no guarantee your process's working directory has any relationship to the scanned repo. If your plugin calls its own file-reading code instead of using the `content` field it was sent, it will not work reliably, and it defeats the entire point of the sandboxing this protocol provides.
...
```json
{"type": "hello", "protocol_version": "1.0", "repoaudit_version": "0.3.0"}
```
...
Echo back the same `path` you were given — RepoAudit uses it to double check the response matches the request it's replying to. If a file produces no findings, reply with `"findings": []`, not a missing/omitted response.

**Finding fields:**

| Field | Required | Notes |
|---|---|---|
| `id` | yes | Stable rule identifier, e.g. `"terraform.public_s3_bucket"`. Should start with `<plugin_name>.` — if it doesn't, RepoAudit prefixes it for you automatically, as a collision guard against another plugin (or a built-in analyzer) using the same bare ID. |
...
| `fix` | yes, non-empty | A concrete, actionable fix — same non-negotiable rule as every built-in RepoAudit rule (see `docs/decisions/` for why). |
...
There is no `file` field on a finding (RepoAudit already knows which file it asked about) and no `category` field required — if omitted, RepoAudit uses your `plugin_name` as the category. There is no `commit_hash` field — plugins don't see git history in this version of the protocol.

Unknown fields on any message are ignored, not rejected — this schema is expected to grow. Don't rely on unknown-field-rejection as validation; that's not how this protocol behaves, deliberately, so old plugins keep working against newer RepoAudit versions where reasonable.
...
`fatal: false` (with a `path`) means this one file couldn't be processed — RepoAudit logs it and moves on to the next file; the rest of the scan, and the rest of your plugin's findings, are unaffected.

`fatal: true` (no `path` needed) means your plugin can't continue at all (e.g. the handshake's `protocol_version` is one you don't support). RepoAudit abandons your plugin for the remainder of the scan — see below.
...
In all three cases: RepoAudit prints one warning naming the plugin and the reason, kills the process if it's still alive, and continues the scan without that plugin's findings for every remaining file. It does not retry, restart, or re-invoke a plugin that has failed once — a plugin that times out or crashes on one file has a real problem that will very likely recur, and retrying adds latency for no expected benefit. This mirrors how RepoAudit already treats other failures it can't fully control (a slow/unreachable OSV.dev API in the Dependency Scanner, a git-history scan that runs out of its time budget): degrade with a clear warning, never let one failing check take down the whole scan.
