...
**Limitation assumée, spécifique à l'usage CLI actuel** : la goroutine abandonnée fuit — elle continue de tourner tant que `a.Run(file)` ne revient pas (jamais, dans le cas d'un vrai hang). Ce n'est pas un problème aujourd'hui parce que `reposcan` est un process one-shot : la goroutine fuyante meurt avec le process quand `main()` retourne, à la fin du scan. Ce ne serait plus vrai si RepoScan était un jour utilisé comme librairie Go dans un process long-vivant (un serveur qui appelle `core.Scanner.Scan()` en boucle, par exemple) — chaque timeout laisserait une goroutine fuir indéfiniment plutôt que de mourir avec le process, une vraie fuite de ressources cumulative. Pas le cas d'usage actuel (CLI uniquement, ni vision.md ni roadmap-long-term.md n'envisagent RepoScan comme librairie embarquée) — donc pas à corriger maintenant, mais si cet usage apparaît un jour, ce compromis devra être revisité en premier (probablement via un `context.Context` annulable propagé dans `Analyzer.Run`, ce qui changerait la signature de l'interface — un changement bien plus lourd qu'aujourd'hui, à ne pas anticiper sans besoin réel).
