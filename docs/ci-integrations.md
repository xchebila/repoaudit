...
reposcan:
  ...
  variables:
    GOPROXY: direct
    GOSUMDB: 'off' # installing reposcan itself from its own repo, not a third-party dependency
  before_script:
    - go install github.com/xchebila/reposcan@main # pin to a release tag once one exists
  script:
    - |
      if [ "$CI_PIPELINE_SOURCE" = "merge_request_event" ]; then
        ...
        reposcan diff "$CI_MERGE_REQUEST_DIFF_BASE_SHA" "$head_sha"
      else
        reposcan scan . --format json | tee reposcan-report.json
      fi
  artifacts:
    when: always
    paths:
      - reposcan-report.json
    expire_in: 1 week
  rules:
...
stage('RepoScan') {
    ...
    steps {
        sh 'go install github.com/xchebila/reposcan@main' // pin to a release tag once one exists
        script {
            if (env.CHANGE_ID) {
                ...
                sh "reposcan diff ${baseSha} ${env.GIT_COMMIT}"
            } else {
                sh 'reposcan scan . --format json | tee reposcan-report.json'
            }
        }
    }
    post {
        always {
            archiveArtifacts artifacts: 'reposcan-report.json', allowEmptyArchive: true
        }
    }
}
```
...
- Unlike the GitHub Action's install step (ADR 0011), there's no "unresolvable synthetic commit" risk here: `reposcan diff` reads whatever's already in the local `.git` object database, and both PR discovery strategies leave a real, locally-present commit for `env.GIT_COMMIT` to point to. What differs between strategies is which commit that is: the Branch Source plugin can be configured to check out either the raw PR head or a Jenkins-built merge of the PR into its target ("Merge Commit" strategy) — know which one your Multibranch job uses, since `reposcan diff` will report against whichever `env.GIT_COMMIT` actually is.
